...
def compute(data, func, data_index={}, 
			threads=multiprocessing.cpu_count(), name=''):
	'''
	Note: This funtion assumes that the df increment is constant 
		  and missing data values are equal to np.nan

	parameters:
		data       : (dict) {index_id: pd.DataFrame(), ...} 
							data that will be computed
		func       : (funct obj) the function each thread will 
								 execute on "data"
		data_index : (dict) data index of ALL given data (if func 
							explicitly needs it)
		threads    : (int) num threads that will be created, 
						   minimum is implicitly 2
		name       : (str) name of computation, only used to print

	NOTE: the "func" parameter is only designed to accept functions 
		  purpose built for the compute() function object argument 

	Assumptions:
		- Each "data" dataframe is paired with the same key used 
		  to access the data_index
	'''
	...
	manager = Manager()
	#determines total computations and creates shared dict "progress"
	...
	#each thread will create an item with key=proc_id and 
	#value=proc_num. Main thread will wipe this after every part
	progress.update({'threads': manager.dict()})

	#the thread_monitor is not included in proc_threads and takes 
	#up one thread
	...
	proc_monitor = Process(target=thread_monitor, 
						   args=(progress, compute_total, threads))
	proc_monitor.start()
	...
	for key, df in data.items():
		...
		#initializes threads with a specified proc interval and 
		#proc_num
		start_index = 0
		last_index = df_len - 1
		procs = []
...
def print_progress_bar(iteration, total, prefix = '', suffix = ''):
	#these varibales used to be parameters but there is no need 
	#to have them change
	length = 49
	fill = '/'
	printEnd = "\r"
...
def thread_monitor(progress, compute_total, thread_total):
	'''
	Parameters:
		progress      : (Manager.dict()) number of computed items 
										 from each thread
	...
	while prog_count < compute_total:
		...
		print_progress_bar(prog_count, compute_total, suffix=suffix)
...
def update_progress(progress, proc_id):
	...
	#creates indicator of this threads existance if one does not exist
...
def trend(interval, df, df_index, progress, proc_id=0):
	'''
	Parameters:
		interval : (list) list of indexes this thread will 
						  compute from df
		df       : (pd.DataFrame()) the DataFrame that this 
									thread will work off
		df_index : (dict) the data index regarding only the 
						  given df
		progress : (Manager.dict()) shared dict for tracking 
									progress across threads
		proc_num : (int) number used to identify each thread 
						 in progress dict

	Assumptions:
		- df.index values are incrementing by 1 continuously
		- df_index['predictions_steps'] is a positive integer 
		  greater than 0
	...
	#slices df so that only df[interval] rows remain in df_slice
	...
	for index, row in df_slice.iterrows():
		...
		if pred_index > last_index:
			#if the next set of future data is not in the bounds 
			#of df, skip this row
			continue

		#creates df of all data needed for computing target val 
		#for current row
		future_df = df.loc[range(index, pred_index), :]

		#isnan is 1 if true and 0 if false so adding row gives 
		#you an isnan total
		isnan_percent = np.sum(future_df.isnan) / pred_steps * 100
		#filters out rows with more than specified number of missing 
		#datapoints (isnan==1)
		if isnan_percent > 10:
			continue

		#drops all rows with missing datapoints (the index at each 
		#row is unchanged)
