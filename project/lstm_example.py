...
'''
The first iteration of this program as I plan it now is to have the graph trend guessing
network output two nodes that determine whether the coin with increase or decrease relative
to fiat reference (USD, EUR, JPY, ...). Depending on difference between how likely it is to 
increase or decrease (softmax activation) and the nets accuracy (calculated using validation 
data) I am hoping to derive magnitude of change. 

For example, if accuracy is 80% and the output is -> (increase_node=.3, decrease_node=.7)
we might expect (remember... just an example and not how it will actually work) the magnitude
of change to be -40% with an expected error range of 20% (AGAIN, NOT ACTUALLY HOW IT WILL WORK;
THIS EXAMPLE WILL CERTAINLY FAIL OR DELIVER BAD RESULTS. This is due to the fact that output of 
networks are strictly class-based and a high increase_node value means only that the network is
confident it will increase but not the degree.)

In order for a network to output that relates to magnitude of change, the network may have to 
have refactored so as to output not only 
'''
...
'''
KINDA UNRELATED: (still in regards to neural networks)

We can write a framework for tensorflow to monitor the behavior of each individual node 
and in order to optimize number of layers, nodes and weights in network.
'''
